...
public class Bounce implements Easing {
    ...
    public double easeIn(double d, double d2, double d3, double d4) {
        return (d3 - easeOut(d4 - d, Utils.DOUBLE_EPSILON, d3, d4)) + d2;
    }

    public double easeInOut(double d, double d2, double d3, double d4) {
        double easeOut;
        if (d < d4 / 2.0d) {
            easeOut = easeIn(d * 2.0d, Utils.DOUBLE_EPSILON, d3, d4) * 0.5d;
        } else {
            easeOut = (easeOut((2.0d * d) - d4, Utils.DOUBLE_EPSILON, d3, d4) * 0.5d) + (d3 * 0.5d);
        }
        return easeOut + d2;
    }
}
