...
public class Entry extends BaseEntry implements Parcelable {
    ...
    public Entry copy() {
        return new Entry(this.f1348x, getY(), getData());
    }

    public boolean equalTo(Entry entry) {
        if (entry != null && entry.getData() == getData() && Math.abs(entry.f1348x - this.f1348x) <= Utils.FLOAT_EPSILON && Math.abs(entry.getY() - getY()) <= Utils.FLOAT_EPSILON) {
            return true;
        }
        return false;
    }
    ...
    public String toString() {
        return "Entry, x: " + this.f1348x + " y: " + getY();
    }

    public void writeToParcel(Parcel parcel, int i) {
        parcel.writeFloat(this.f1348x);
