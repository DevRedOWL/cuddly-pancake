...
final class MarkableInputStream extends InputStream {
    ...
    /* renamed from: in */
    private final InputStream f2305in;
    private long limit;
    ...
    private MarkableInputStream(InputStream inputStream, int i, int i2) {
        ...
        this.f2305in = !inputStream.markSupported() ? new BufferedInputStream(inputStream, i) : inputStream;
    ...
    private void setLimit(long j) {
        try {
            if (this.reset >= this.offset || this.offset > this.limit) {
                ...
                this.f2305in.mark((int) (j - this.offset));
            } else {
                this.f2305in.reset();
                this.f2305in.mark((int) (j - this.reset));
    ...
    private void skip(long j, long j2) throws IOException {
        while (j < j2) {
            long skip = this.f2305in.skip(j2 - j);
            if (skip == 0) {
                if (read() != -1) {
                    skip = 1;
                } else {
    ...
    public int read(byte[] bArr) throws IOException {
        ...
        int read = this.f2305in.read(bArr);
        if (read != -1) {
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        ...
        int read = this.f2305in.read(bArr, i, i2);
        if (read != -1) {
            this.offset += (long) read;
        }
        return read;
    ...
    public long skip(long j) throws IOException {
        ...
        long skip = this.f2305in.skip(j);
        this.offset += skip;
        return skip;
    }

    public int available() throws IOException {
        return this.f2305in.available();
    }
    ...
    public boolean markSupported() {
        return this.f2305in.markSupported();
    }
}
