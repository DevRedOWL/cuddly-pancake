...
class DefaultSettingsJsonTransform implements SettingsJsonTransform {
    ...
    public SettingsData buildFromJson(CurrentTimeProvider currentTimeProvider, JSONObject jSONObject) throws JSONException {
        ...
        int optInt2 = jSONObject.optInt("cache_duration", NikonType2MakernoteDirectory.TAG_NIKON_SCAN);
        return new SettingsData(getExpiresAtFrom(currentTimeProvider, (long) optInt2, jSONObject), buildAppDataFrom(jSONObject.getJSONObject(GCMConstants.EXTRA_APPLICATION_PENDING_INTENT)), buildSessionDataFrom(jSONObject.getJSONObject("session")), buildFeaturesSessionDataFrom(jSONObject.getJSONObject("features")), optInt, optInt2);
    }

    static Settings defaultSettings(CurrentTimeProvider currentTimeProvider) {
        JSONObject jSONObject = new JSONObject();
        return new SettingsData(getExpiresAtFrom(currentTimeProvider, 3600, jSONObject), (AppSettingsData) null, buildSessionDataFrom(jSONObject), buildFeaturesSessionDataFrom(jSONObject), 0, NikonType2MakernoteDirectory.TAG_NIKON_SCAN);
    }

    public JSONObject toJson(SettingsData settingsData) throws JSONException {
        return new JSONObject().put("expires_at", settingsData.expiresAtMillis).put("cache_duration", settingsData.cacheDuration).put("settings_version", settingsData.settingsVersion).put("features", toFeaturesJson(settingsData.featuresData)).put(GCMConstants.EXTRA_APPLICATION_PENDING_INTENT, toAppJson(settingsData.appData)).put("session", toSessionJson(settingsData.sessionData));
    }
