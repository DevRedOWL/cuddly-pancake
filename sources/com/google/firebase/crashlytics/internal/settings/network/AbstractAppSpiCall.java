...
abstract class AbstractAppSpiCall extends AbstractSpiCall implements AppSpiCall {
    ...
    public boolean invoke(AppRequestData appRequestData, boolean z) {
        if (z) {
            ...
            logger.mo34251d("Sending app info to " + getUrl());
            try {
                HttpResponse execute = applyMultipartDataTo.execute();
                int code = execute.code();
                String str = HttpPost.METHOD_NAME.equalsIgnoreCase(applyMultipartDataTo.method()) ? "Create" : "Update";
                Logger logger2 = Logger.getLogger();
                logger2.mo34251d(str + " app request ID: " + execute.header(AbstractSpiCall.HEADER_REQUEST_ID));
                Logger logger3 = Logger.getLogger();
                logger3.mo34251d("Result was " + code);
                return ResponseParser.parse(code) == 0;
            } catch (IOException e) {
                Logger.getLogger().mo34254e("HTTP request failed.", e);
