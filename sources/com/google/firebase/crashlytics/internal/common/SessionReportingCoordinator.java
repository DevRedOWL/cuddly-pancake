...
class SessionReportingCoordinator implements CrashlyticsLifecycleEvents {
    ...
    public void persistUserId(String str) {
        ...
        if (userId == null) {
            Logger.getLogger().mo34251d("Could not persist user ID; no user ID available");
        } else {
    ...
    public Task<Void> sendReports(Executor executor, DataTransportState dataTransportState) {
        if (dataTransportState == DataTransportState.NONE) {
            Logger.getLogger().mo34251d("Send via DataTransport disabled. Removing DataTransport reports.");
            this.reportPersistence.deleteAllReports();
        ...
        for (CrashlyticsReportWithSessionId next : loadFinalizedReports) {
            if (next.getReport().getType() != CrashlyticsReport.Type.NATIVE || dataTransportState == DataTransportState.ALL) {
                ...
                Logger.getLogger().mo34251d("Send native reports via DataTransport disabled. Removing DataTransport reports.");
    ...
    private void persistEvent(Throwable th, Thread thread, String str, String str2, long j, boolean z) {
        ...
        if (logString != null) {
            ...
            Logger.getLogger().mo34251d("No log data to include with this event.");
        }
    ...
    public boolean onReportSendComplete(Task<CrashlyticsReportWithSessionId> task) {
        if (task.isSuccessful()) {
            ...
            Logger logger = Logger.getLogger();
            logger.mo34251d("Crashlytics report successfully enqueued to DataTransport: " + result.getSessionId());
            this.reportPersistence.deleteFinalizedReport(result.getSessionId());
            return true;
        }
        Logger.getLogger().mo34252d("Crashlytics report could not be enqueued to DataTransport", task.getException());
        return false;
    }
