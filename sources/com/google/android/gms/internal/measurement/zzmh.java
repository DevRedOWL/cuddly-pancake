...
public final class zzmh implements zzme {
    ...
    static {
        ...
        zzb = zzdm.zza("measurement.config.cache_time", (long) DateUtils.MILLIS_PER_DAY);
        zzc = zzdm.zza("measurement.log_tag", "FA");
        zzd = zzdm.zza("measurement.config.url_authority", "app-measurement.com");
        zze = zzdm.zza("measurement.config.url_scheme", UriUtil.HTTPS_SCHEME);
        ...
        zzk = zzdm.zza("measurement.alarm_manager.minimum_interval", (long) DateUtils.MILLIS_PER_MINUTE);
        zzl = zzdm.zza("measurement.upload.minimum_delay", 500);
        zzm = zzdm.zza("measurement.monitoring.sample_period_millis", (long) DateUtils.MILLIS_PER_DAY);
        zzn = zzdm.zza("measurement.upload.realtime_upload_interval", 10000);
        zzo = zzdm.zza("measurement.upload.refresh_blacklisted_config_interval", (long) GCMRegistrar.DEFAULT_ON_SERVER_LIFESPAN_MS);
        zzp = zzdm.zza("measurement.config.cache_time.service", (long) DateUtils.MILLIS_PER_HOUR);
        zzq = zzdm.zza("measurement.service_client.idle_disconnect_millis", 5000);
        zzr = zzdm.zza("measurement.log_tag.service", "FA-SVC");
        zzs = zzdm.zza("measurement.upload.stale_data_deletion_interval", (long) DateUtils.MILLIS_PER_DAY);
        zzt = zzdm.zza("measurement.upload.backoff_period", 43200000);
        zzu = zzdm.zza("measurement.upload.initial_upload_delay_time", 15000);
        zzv = zzdm.zza("measurement.upload.interval", (long) DateUtils.MILLIS_PER_HOUR);
        zzw = zzdm.zza("measurement.upload.max_bundle_size", 65536);
        ...
        zzaf = zzdm.zza("measurement.upload.max_batch_size", 65536);
        zzag = zzdm.zza("measurement.upload.retry_count", 6);
        zzah = zzdm.zza("measurement.upload.retry_time", (long) CycledLeScanner.ANDROID_N_MAX_SCAN_DURATION_MILLIS);
        zzai = zzdm.zza("measurement.upload.url", "https://app-measurement.com/a");
        zzaj = zzdm.zza("measurement.upload.window_interval", (long) DateUtils.MILLIS_PER_HOUR);
    }
}
