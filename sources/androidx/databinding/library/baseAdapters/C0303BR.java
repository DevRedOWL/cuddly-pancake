package androidx.databinding.library.baseAdapters;

/* renamed from: androidx.databinding.library.baseAdapters.BR */
public class C0303BR {
    public static final int _all = 0;
    public static final int active = 22;
    public static final int arrow = 4;
    public static final int backIsVisible = 8;
    public static final int binder = 11;
    public static final int btnTitle = 7;
    public static final int color = 3;
    public static final int editMode = 30;
    public static final int enable = 9;
    public static final int hint = 28;
    public static final int inputType = 10;
    public static final int isCheck = 24;
    public static final int isDemoButtonEnabled = 1;
    public static final int isEditMode = 21;
    public static final int isEnabled = 29;
    public static final int isRequired = 16;
    public static final int key = 14;
    public static final int label = 23;
    public static final int name = 31;
    public static final int pass = 20;
    public static final int position = 32;
    public static final int res = 17;
    public static final int resourc = 15;
    public static final int result = 6;
    public static final int selected = 13;
    public static final int size = 25;
    public static final int subtitle = 27;
    public static final int ticket = 19;
    public static final int title = 5;
    public static final int value = 12;
    public static final int visible = 18;
    public static final int visibleArrow = 2;

    /* renamed from: vm */
    public static final int f66vm = 26;
}
