package p035ru.unicorn.ujin.view.fragments.chats;

import android.content.DialogInterface;
import kotlin.Metadata;
import kotlin.jvm.internal.Intrinsics;
import p035ru.unicorn.ujin.data.realm.Chat;
import p035ru.unicorn.ujin.view.fragments.chats.viewModel.chatViewModel.ChatsViewModel;

@Metadata(mo51341bv = {1, 0, 3}, mo51342d1 = {"\u0000\u0016\n\u0000\n\u0002\u0010\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0002\n\u0002\u0010\b\n\u0000\u0010\u0000\u001a\u00020\u00012\u000e\u0010\u0002\u001a\n \u0004*\u0004\u0018\u00010\u00030\u00032\u0006\u0010\u0005\u001a\u00020\u0006H\n¢\u0006\u0002\b\u0007"}, mo51343d2 = {"<anonymous>", "", "dialog", "Landroid/content/DialogInterface;", "kotlin.jvm.PlatformType", "<anonymous parameter 1>", "", "onClick"}, mo51344k = 3, mo51345mv = {1, 4, 1})
/* renamed from: ru.unicorn.ujin.view.fragments.chats.ChatBaseFragment$deleteChat$1 */
/* compiled from: ChatBaseFragment.kt */
final class ChatBaseFragment$deleteChat$1 implements DialogInterface.OnClickListener {
    final /* synthetic */ Chat $chat;
    final /* synthetic */ String $userId;
    final /* synthetic */ ChatBaseFragment this$0;

    ChatBaseFragment$deleteChat$1(ChatBaseFragment chatBaseFragment, Chat chat, String str) {
        this.this$0 = chatBaseFragment;
        this.$chat = chat;
        this.$userId = str;
    }

    public final void onClick(DialogInterface dialogInterface, int i) {
        if (Intrinsics.areEqual((Object) this.$chat.getOwnerId(), (Object) this.$userId)) {
            ChatsViewModel chatViewModel = this.this$0.getChatViewModel();
            String id = this.$chat.getId();
            Intrinsics.checkNotNullExpressionValue(id, "chat.id");
            chatViewModel.requestDeleteChat(id);
        } else {
            ChatsViewModel chatViewModel2 = this.this$0.getChatViewModel();
            String id2 = this.$chat.getId();
            Intrinsics.checkNotNullExpressionValue(id2, "chat.id");
            chatViewModel2.requestExitChat(id2);
        }
        dialogInterface.dismiss();
    }
}
