...
import p042cz.msebera.android.httpclient.MalformedChunkCodingException;
import p042cz.msebera.android.httpclient.TruncatedChunkException;
...
public class ChunkedInputStream extends InputStream {
    ...
    /* renamed from: in */
    private final SessionInputBuffer f5702in;
    private int pos;
    private int state;

    public ChunkedInputStream(SessionInputBuffer sessionInputBuffer) {
        this.f5702in = (SessionInputBuffer) Args.notNull(sessionInputBuffer, "Session input buffer");
        this.pos = 0;
    ...
    public int available() throws IOException {
        SessionInputBuffer sessionInputBuffer = this.f5702in;
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        if (this.closed) {
            ...
            int read = this.f5702in.read(bArr, i, Math.min(i2, this.chunkSize - this.pos));
            if (read != -1) {
                this.pos += read;
                if (this.pos >= this.chunkSize) {
                    this.state = 3;
    ...
    private int getChunkSize() throws IOException {
        ...
        if (i != 1) {
            if (i == 3) {
                this.buffer.clear();
                if (this.f5702in.readLine(this.buffer) == -1) {
                    return 0;
                }
    ...
    private void parseTrailerHeaders() throws IOException {
        try {
            this.footers = AbstractMessageParser.parseHeaders(this.f5702in, -1, -1, (LineParser) null);
