...
public class Header implements Iterable<MinimalField> {
    ...
    public MinimalField getField(String str) {
        List list;
        if (str == null || (list = this.fieldMap.get(str.toLowerCase(Locale.ENGLISH))) == null || list.isEmpty()) {
            return null;
        }
    ...
    public List<MinimalField> getFields(String str) {
        ...
        List list = this.fieldMap.get(str.toLowerCase(Locale.ENGLISH));
    ...
    public int removeFields(String str) {
        List remove;
        if (str == null || (remove = this.fieldMap.remove(str.toLowerCase(Locale.ENGLISH))) == null || remove.isEmpty()) {
            return 0;
        }
    ...
    public void setField(MinimalField minimalField) {
        if (minimalField != null) {
            List list = this.fieldMap.get(minimalField.getName().toLowerCase(Locale.ENGLISH));
            if (list == null || list.isEmpty()) {
