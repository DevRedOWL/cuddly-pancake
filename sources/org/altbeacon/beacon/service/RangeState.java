    ...
    public void addBeacon(Beacon beacon) {
        ...
        if (rangedBeacon != null) {
            if (LogManager.isVerboseLoggingEnabled()) {
                LogManager.m6834d(TAG, "adding %s to existing range for: %s", beacon, rangedBeacon);
            }
            rangedBeacon.updateBeacon(beacon);
            return;
        }
        if (LogManager.isVerboseLoggingEnabled()) {
            LogManager.m6834d(TAG, "adding %s to new rangedBeacon", beacon);
        }
            ...
            for (Beacon next : this.mRangedBeacons.keySet()) {
                ...
                if (rangedBeacon != null) {
                    ...
                    if (!rangedBeacon.noMeasurementsAvailable()) {
                        ...
                        LogManager.m6834d(TAG, "Dumping beacon from RangeState because it has no recent measurements.", new Object[0]);
                    }
                }
            }
