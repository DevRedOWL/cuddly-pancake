...
public class Mat {
    ...
    public Mat(Mat mat, Rect rect) {
        this.nativeObj = n_Mat(mat.nativeObj, rect.f6487y, rect.height + rect.f6487y, rect.f6486x, rect.f6486x + rect.width);
    }
    ...
    public void locateROI(Size size, Point point) {
        ...
        if (point != null) {
            point.f6481x = dArr2[0];
            point.f6482y = dArr2[1];
        }
    }
    ...
    public Mat submat(Rect rect) {
        return new Mat(n_submat(this.nativeObj, rect.f6486x, rect.f6487y, rect.width, rect.height));
    }
